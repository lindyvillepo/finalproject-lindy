...
## Test that if you create a card instance, it will have access to a variable suit_names that contains the list ["Diamonds","Clubs","Hearts","Spades"]
	def test_suit_name(self):
		new = Card()
		self.assertEqual(new.suit_names, ['Diamonds','Clubs','Hearts','Spades'], "Testing suit_names of Card class")
## Test that if you invoke the __str__ method of a card instance that is created with suit=2, rank=7, it returns the string "7 of Hearts"
	def test_string(self):
		test = Card(2, 7)
		self.assertEqual(str(test), "7 of Hearts", "Testing string of Card class")
## Test that if you create a deck instance, it will have 52 cards in its cards instance variable
	def test_cards(self):
		deck = Deck()
		length = deck.cards
		self.assertEqual(len(length), 52, "Testing Deck class has 52 cards")
## Test that if you invoke the pop_card method on a deck, it will return a card instance.
	def test_pop_card(self):
		pop = Deck()
		pop1 = pop.pop_card(i=-1)
		self.assertEqual(type(pop1), type(Card()), "Testing pop_card")
...
## Write at least 2 additional tests (not repeats of the above described tests). Make sure to include a descriptive message in these two so we can easily see what you are testing!
	# Test that the shuffle function will return a different deck than before
	def test_shuffle(self):
		shuffle1 = Deck()
		shuffle2 = shuffle1.shuffle
		self.assertNotEqual(shuffle1, shuffle2, "Testing shuffle")
	# Test that if you create a card instance, it can access the variable faces that contain the dictionary {1:"Ace",11:"Jack",12:"Queen",13:"King"}
	def test_faces(self):
		card1 = Card()
		self.assertEqual(card1.faces, {1:"Ace",11:"Jack",12:"Queen",13:"King"}, "Testing faces variable")
