...
user += 'Users (user_id TEXT PRIMARY KEY, screen_name TEXT, num_favs INTEGER, description TEXT)'
cur.execute(user)
...
tweet_info = list(zip(tweet_id, tweet_text, created, retweets, user_post))
...
user = 'INSERT OR IGNORE INTO Users VALUES (?, ?, ?, ?)'
for u in user_info:
	cur.execute(user, u)
...
query1 = "SELECT screen_name FROM Users"
cur.execute(query1)
tuple_name = cur.fetchall()
screen_names = [x[0] for x in tuple_name]
...
query2 = "SELECT * FROM Tweets WHERE retweets > 25"
cur.execute(query2)
...
query3 = "SELECT description FROM Users WHERE num_favs > 25"
cur.execute(query3)
tuple_description = cur.fetchall()
descriptions_fav_users = [x[0] for x in tuple_description]
...
query4 = "SELECT Users.screen_name, Tweets.text FROM Tweets INNER JOIN Users on Tweets.user_id = Users.user_id WHERE Tweets.retweets > 5"
cur.execute(query4)
...
description_words = {word for word in word_list}
...
query5 = "SELECT Users.screen_name, Tweets.text FROM Tweets INNER JOIN Users on Tweets.user_id = Users.user_id"
cur.execute(query5)
...
twitter_info_diction = dict(twitter_default)
