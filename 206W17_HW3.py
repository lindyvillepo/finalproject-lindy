...
## Use regular expressions to define a function called parse_counted_words which should accept a string, find strings of the form: <count> <word>  e.g. 101 Dalmations, inside it, and should return either 
## the LAST correctly matching number-word combo in the string as a tuple, e.g. ('13', "pineapples"), if there are any such sub-strings, or None, if there are not.
...
def parse_counted_words(s):
    word = re.findall(r'(\d+)\s(\W[A-Za-z]+|[A-Za-z]+)', s)
    if len(word) == 0:
        return None
    else:
        return word[-1]
...
file2 = re.findall(r"(\/Users|~).+", data)
full_paths_num = len(file2)
...
file3 = re.findall(r".*SI206.+\.py", data)
python_course_paths = len(file3)
...
file4 = re.findall(r".+\d+(\.docx|\.xlsx)", data) 
microsoft_files_num = len(file4)
